...
public interface IHechosEstaticosRepository extends JpaRepository<Hecho, Long> {
    ...
    @Query("SELECT h.provincia, COUNT(h) FROM Hecho h WHERE h.eliminado = false GROUP BY h.provincia")
    List<Object[]> contarHechosPorProvincia();

    @Query("SELECT h.categoria, COUNT(h) FROM Hecho h WHERE h.eliminado = false GROUP BY h.categoria")
    List<Object[]> contarHechosPorCategoria();

    // Si seguís necesitando “no sincronizados”, podés derivarlo de algún flag que tengas
    @Query("SELECT h FROM Hecho h WHERE h.eliminado = false") // ejemplo neutral
